...
def calc_dist_cupy(vegt_clip_gpu, cond_clip_gpu):
    vegt = cp.asarray(vegt_clip_gpu) 
    cond = cp.asarray(cond_clip_gpu) 
    
    # start_time = time.time()
    min_values = []
    
    for x in vegt:
        dist = cp.min(cp.sqrt(cp.sum(cp.square(x - cond),axis=1)))
        min_values.append(dist)

    # epoch_mins, epoch_secs = epoch_time(start_time, time.time())
    # logging.info(f'{vegt_clip_gpu.shape},{cond_clip_gpu.shape},{epoch_mins}m {epoch_secs}s') 

    min_dist = cp.expand_dims(cp.array(min_values), axis=1)
    all_data = cp.append(vegt, min_dist, axis=1)

    cp.savetxt('results/all_data_cupy.txt', all_data, delimiter=',', fmt='%f')
...
def main(args):
    ...
    logging.basicConfig(filename="results/cupy"+"_runtime.log", level=logging.DEBUG)
    ...
    for filename in tqdm.tqdm(csv_files[:limit]):
        ...
        with open("results/cupy_"+str(limit)+"_metadata.txt", "a") as file_object:
        ...
        calc_dist_cupy(vegt_clip, cond_clip)

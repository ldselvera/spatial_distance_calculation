...
def calc_dist_tf(vegt_clip_gpu, cond_clip_gpu):
    vegt = tf.convert_to_tensor(vegt_clip_gpu)
    cond = tf.convert_to_tensor(cond_clip_gpu)
    
    # start_time = time.time()
    min_values = []

    for x in vegt:
        dist = tf.reduce_min(tf.sqrt(tf.reduce_sum(tf.square(tf.subtract(x, cond)), 1)))
        min_values.append(dist)

    # epoch_mins, epoch_secs = epoch_time(start_time, time.time())
    # logging.info(f'{vegt_clip_gpu.shape},{cond_clip_gpu.shape},{epoch_mins}m {epoch_secs}s') 

    min_dist = tf.expand_dims(tf.stack(min_values, 0),1)
    all_data = tf.concat((vegt,min_dist), -1 )
    cp.savetxt('results/all_data_tf.txt', all_data, delimiter=',', fmt='%f')    
...
def main(args):
    ...
    logging.basicConfig(filename="results/tf_runtime.log", level=logging.DEBUG)
    ...
    for filename in tqdm.tqdm(csv_files[:limit]):
        ...
        with open("results/tf_"+str(limit)+"_metadata.txt", "a") as file_object:
        ...
        calc_dist_tf(vegt_clip, cond_clip)
